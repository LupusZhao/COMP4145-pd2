...
def is_crypto(symbol):
    """Check if it is a cryptocurrency"""
    return "-USD" in symbol or symbol in ["BTC", "ETH", "BNB"]

def is_etf(symbol):
    """Check if it is an ETF"""
    etf_list = ["SPY", "VOO", "QQQ", "IWM", "DIA"]  # Common ETF list
...
def safe_get_ticker_info(symbol):
    """Safely get stock information with retry mechanism"""
    try:
...
with tabs[0]:
    ...
    with col1:
            ...
            try:
                ...
                if info and "symbol" in info:
                    ...
                    if symbol not in st.session_state["your_picks"]:
                        ...
                        total = len(st.session_state["your_picks"])
                        for s in st.session_state["your_picks"]:
                            st.session_state["pick_weights"][s] = 100.0 / total
                        st.success(f"Added: {symbol} ({info.get('shortName', '')})")
                    else:
        ...
        # Selected assets and weight settings
        ...
        if st.session_state["your_picks"]:
            ...
            for i, symbol in enumerate(st.session_state["your_picks"]):
                ...
                if col_w[2].button("Delete", key=f"del_{symbol}"):
                    ...
                    del st.session_state["pick_weights"] [symbol]
                    st.rerun()
    ...
    # Right: Broker sync and CSV import
    with col2:
        ...
        if uploaded_file:
            try:
                ...
                if len(df.columns) >= 1:
                    # Handle asset column
                    holdings = df.iloc[:, 0].tolist()
                    # Handle weight column
                    weights = {}
                    if len(df.columns) >= 2:
                        weights = dict(zip(holdings, df.iloc[:, 1].tolist()))
                        ...
                        # Average weights
                        weight_val = 100.0 / len(holdings)
                        weights = {s: weight_val for s in holdings}

                    st.write("Import Preview:")
                    st.dataframe(pd.DataFrame({"Asset": holdings, "Weight (%)": [weights[s] for s in holdings]}))
                    ...
                    st.error("CSV format error, must have at least one column for asset symbol.")
            except Exception as e:
        ...
        if st.session_state["portfolios"]:
            for i, portfolio in enumerate(st.session_state["portfolios"]):
                ...
                if col_p[1].button("Switch", key=f"switch_{i}"):
                    ...
                    st.success(f"Switched to {portfolio['name']}")
                if col_p[2].button("Delete", key=f"del_port_{i}"):
                    st.session_state["portfolios"].pop(i)
                    if st.session_state["current_portfolio"] == portfolio["name"]:
                        st.session_state["current_portfolio"] = None
    ...
    try:
        ...
        for item in data.get("news", []):
            ...
            if filter_type != "All Holdings":
                if filter_type == "Stocks" and asset_type != "股票":
                    continue
                elif filter_type == "ETFs" and asset_type != "ETF":
                    continue
                elif filter_type == "Cryptocurrencies" and asset_type != "加密货币":
                    continue
            news_list.append({
...
with tabs[1]:
    ...
    with right:
        ...
        if news:
            ...
            with col_font[0]:
                if st.button("A+"):
                    st.session_state["font_size"] = min(st.session_state["font_size"] + 2, 24)
                    st.rerun()
            with col_font[1]:
                if st.button("A-"):
                    st.session_state["font_size"] = max(st.session_state["font_size"] - 2, 8)
                    st.rerun()
            st.markdown(
...
def get_market_cap(symbol):
    ...
    if info and "marketCap" in info and info["marketCap"] is not None:
        return round(info["marketCap"] / 1e8, 1)  # 转换为亿美元
...
def get_dividend(symbol):
    ...
    if info and "dividendYield" in info and info["dividendYield"] is not None:
        return round(info["dividendYield"] * 100, 2)  # 转换为百分比
...
with tabs[2]:
    ...
    col_metrics[0].metric("Portfolio Beta", round(portfolio_beta, 2), 
                         help="Measures correlation with market. >1 means more volatile than market.")
    col_metrics[1].metric("Portfolio P/E", round(portfolio_pe, 2) if portfolio_pe else "N/A",
                         help="Reflects valuation level. Lower may mean cheaper.")
    col_metrics[2].metric("Portfolio Dividend Yield (%)", round(portfolio_div, 2),
                         help="Annual dividend as a percentage of price.")
    ...
    try:
        with st.spinner(f"获取{symbol}数据中..."):
            ...
            if not hist.empty:
                ...
                with col_fin[0]:
                    st.write(f"市值: {get_market_cap(symbol):,} 亿美元")
                with col_fin[1]:
                    st.write(f"市盈率: {get_pe(symbol)}")
                with col_fin[2]:
                    st.write(f"贝塔值: {get_beta(symbol)}")
                ...
                if "5日均线" in studies:
                    ax.plot(hist.index, hist["Close"].rolling(5).mean(), label="5-day MA", color='orange', alpha=0.7)
                if "10日均线" in studies:
                    ax.plot(hist.index, hist["Close"].rolling(10).mean(), label="10-day MA", color='purple', alpha=0.7)
                if "20日均线" in studies:
                    ax.plot(hist.index, hist["Close"].rolling(20).mean(), label="20-day MA", color='green', alpha=0.7)
                if "60日均线" in studies:
                    ax.plot(hist.index, hist["Close"].rolling(60).mean(), label="60-day MA", color='red', alpha=0.7)
                ...
                if news:
                    for n in news[:5]:
                        publish_time = datetime.fromtimestamp(n.get("providerPublishTime", 0)/1000).strftime('%Y-%m-%d')
                        st.write(f"**{n.get('title', '')}**")
                        st.write(f"Source: {n.get('publisher', '')} | Date: {publish_time}")
                        st.markdown(f"[Read original]({n.get('link', '')})")
                        st.divider()
                else:
                    st.info("No related news available.")
            else:
        ...
        if selected_port:
            ...
            for i, s in enumerate(port_holdings):
                try:
                    ...
                    if not h.empty:
                        p = h["Close"].iloc[-1]
                        prev_p = h["Close"].iloc[-2] if len(h) > 1 else p
                        chg = p - prev_p
                        pct = (chg / prev_p * 100) if prev_p else 0
                        cols[i % 3].metric(f"{s}", f"${p:.2f}", delta=f"{chg:+.2f} ({pct:+.2f}%)")
                    else:
                        cols[i % 3].metric(f"{s}", "No data")
                except:
                    cols[i % 3].metric(f"{s}", "Fetch failed")
    else:
...
# Retrieve News
def retrieve_news():
    url = "https://api.worldnewsapi.com/retrieve-news?ids=2352,2354"
    api_key = "6d6d5f5d55b64eab94814259cb0a0841"
    headers = {'x-api-key': api_key}
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
...
# Extract News
def extract_news():
    url = "https://api.worldnewsapi.com/extract-news?url=https://nypost.com/2024/04/02/us-news/lawmakers-demand-to-know-if-taxpayers-funded-al-shabaab-terrorist/"
    api_key = "6d6d5f5d55b64eab94814259cb0a0841"
    headers = {'x-api-key': api_key}
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
